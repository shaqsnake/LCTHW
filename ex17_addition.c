...
void Database_set(struct Connection *conn, int id, const char *name, const char *email)
{
    ...
    addr->set = 1;
    // WARNING: bug, read the "How to Break it" and fix thix
...
int main(int argc, char *argv[])
{
    ...
    if (argc>3 && argc != 5) id = atoi(argv[3]);
    if (argc==5) {
        attribute = argv[3];
        value = argv[4];
    }
    ...
    switch (action) {
        ...
        case 'f':
            if (argc!=5) die("Need attribute and value to find", conn);
        ...
        case 'd':
            if (argc!=4) die("Need id to handle", conn);
