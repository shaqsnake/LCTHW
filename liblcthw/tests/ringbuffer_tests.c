...
char *test_create()
{
    buffer = RingBuffer_create(DEFAULT_BUFFER_LENGTH);
    mu_assert(buffer != NULL, "Failed to create ringbuffer.");
    //mu_assert(buffer->length = DEFAULT_BUFFER_LENGTH, "Wrong length.");
...
char *test_read_write()
{
    ...
    mu_assert(RingBuffer_available_data(buffer) == 100, "Invaild available data.");
    mu_assert(RingBuffer_available_space(buffer) == 0, "Invaild available space.");
    ...
    mu_assert(RingBuffer_available_space(buffer) == 100 - 6, "Should have 94 data.");
